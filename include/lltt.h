...
#define __builtin_rvtt_ttinsn(DYNAMIC, ENCODING) \
  __builtin_rvtt_ttinsn(lltt::__instrn_buffer, DYNAMIC, ENCODING)
...
[[gnu::always_inline]] inline void insn(uint32_t insn) {
  __builtin_rvtt_ttinsn(Dynamic, insn);
}

// For use by ckernel_ops.h expansion
#define LLTT_INSN(DYNAMIC, ENCODING) ::lltt::insn<DYNAMIC>(ENCODING)
