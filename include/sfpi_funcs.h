...
template<template<int> typename Derived, int Mod>
auto sfpi::impl_::vReg_<Derived, Mod>::operator= (vFloat16a val) const-> vFloat16a {
...
template<template<int> typename Derived, int Mod>
auto sfpi::impl_::vReg_<Derived, Mod>::operator= (vFloat16b val) const-> vFloat16b {
...
template<template<int> typename Derived, int Mod>
auto sfpi::impl_::vReg_<Derived, Mod>::operator= (vInt16 val) const-> vInt16 {
  constexpr int mod = Mod >= 0 ? Mod : SFPSTORE_MOD0_FMT_INT16;
  static_assert (false
                 || mod == SFPSTORE_MOD0_FMT_INT16
                 || mod == SFPSTORE_MOD0_FMT_INT8
                 , "Mod value not compatible with storing vInt");
  write (val.get (), mod);
  return val;
}
...
template<template<int> typename Derived, int Mod>
auto sfpi::impl_::vReg_<Derived, Mod>::operator= (vUInt16 val) const-> vUInt16 {
