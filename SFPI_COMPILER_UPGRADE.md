...
**F1.0 oracle audit (2026-08-15): FAILED CALIBRATION GATE.** CRAQ commit `f2d145ea` restores the `simulated_cycles` field that rollback `747e0c95` removed while leaving the extractor schema behind. The repaired model reports the same scoped `WELFORD_BODY` at 376 cycles for handwritten replay and 284 for generated vFloat; silicon reports 323 and 326 respectively. CRAQ therefore predicts the wrong winner (`-24.5%` versus silicon's `+0.9%` for vFloat relative to replay). Aggregate instruction/stall totals cannot repair this because Tensix pipes progress concurrently. Until the SFPU timing model is calibrated, CRAQ is a functional/debugging oracle and silicon is the performance acceptance authority; compiler scheduling decisions must not be trained to this mismatched result.
...
**Reduce-SDPA discriminator (2026-08-15).** TT-Metal `6d7c0fdb` adds a test-only identical-math handwritten-replay/generated-SFPI selector and a serialized Blackhole profiler archive without changing the production LLK. Both paths pass the full 512x64 four-subblock golden. The handwritten 8-slot replay body measures `839,839,839` `REDUCE_SDPA_BODY` device cycles; generated SFPI measures `914,914,914` (`+75`, `+8.94%`). The generated ELF is smaller and CRAQ retires the same SFPU/total instruction counts, but the final code has no replay: static body delivery is handwritten `TTREPLAY=65, SFPLOAD=17, SFPSWAP=12` versus generated `TTREPLAY=0, SFPLOAD=71, SFPSWAP=72`. This is a concrete general Track-D target: late post-RA exact-encoding replay formation, not a kernel-specific source rewrite. Artifacts and hashes are recorded in TT-Metal `tt_metal/tt-llk/tests/corpus/REDUCE_SDPA_SILICON_AB.md`.
