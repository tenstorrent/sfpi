...
# $FILE HASH $dir -- emit variable initializations from hash file(s)
...
if [[ ${#1} = 0 ]]; then
    cat >&2 <<EOF
Usage:
$0 HASH [\$DIR]	 - generate release information
...
if [[ ${1-} = HASH ]]; then
    ...
    sed 's/^\([0-9a-f]*\) \*sfpi_[^_]*_\([^.]*\)\.\(.*\)$/sfpi_\2_\3_hash=\1/' ${2-.}/*.$hashtype >>$version_file
